...
- 每个 GPIO/UART/TIMER/SPI/I2C/WDT/PWM/IRQCTRL/SYSCTRL 外设的 RTL 定向测试；
...
'gpio','timer','uart','spi','i2c','wdt','pwm','irqctrl','sysctrl','system','system-uart',
'sdk-isa','sdk-peripherals','sdk-i2c','sdk-irq','tn9k','tn9k-wdt','tn9k-peripherals' |
...
2026-08-25，当前 v0.4 外部中断 RTL 使用 `omcu_irq_smoke` 在完全相同的
`GW1NR-LV9QN88PC6/I5`、`GW1N-9C`、8 KiB ROM、44 KiB SRAM、Yosys 0.68、
nextpnr-himbaechel-gowin 0.11.1 与 Apycula 0.32 条件下完成了综合、P&R 和 `.fs`
打包。最终 report 为 `system.clk_i = 45.554 MHz`（约束 27 MHz，15.085 ns 余量），
资源为 5,892/8,640 LUT4、1,643/6,480 DFF、26/26 BSRAM、1,056/6,480 ALU、
1/5 MULT36X36 和 15/276 IOB。manifest 已确认四个 Boot ROM BSRAM 的综合/P&R
初始化指纹完全相同。

下表中旧的 board demo 与 peripheral smoke 行是 v0.4 IRQCTRL 接入前的历史
ROM 选择基线，保留它们用于审计，不把它们冒充为当前 IRQ 版本的实现结果。

| SDK ROM | 输入 SHA-256 | 综合/P&R BSRAM 初始化指纹 | `.fs` SHA-256 |
| --- | --- | --- | --- |
| `omcu_irq_smoke`（当前 v0.4） | `1409af0b9d1a1498520e6378752a2959c7d58979a4d5f0c232fa5bdd253d0b4d` | `173d1cf6c36fc89aedc62a7e5bff39cb255e064d2bfccaa616ec0bc604295c82` | `71e660f93b7ff190adfebffc697944b03c5175309f7bb5523a811448de5f5395` |
...
P&R 网表的四个 Boot ROM BSRAM `INIT_RAM_xx` 参数产生指纹；两者不同即失败。当前
`omcu_irq_smoke` 行证明中断 SDK 程序真正进入了 P&R 后的 FPGA 配置，而不只是命令行中
传入不同文件名；旧两行彼此不同的输入 SHA、BRAM 指纹和 `.fs` SHA 则保留了 ROM
选择的历史对比证据。

...
2. 如果有可用应用，启动器给 UART 更新器约 750 ms 的连接窗口；没有连接则复制镜像到 SRAM 并跳转到应用复位向量。
3. 如果没有任何有效应用，启动器会持续监听 UART，便于空白设备恢复。
4. 更新时，启动器先擦除**非当前有效槽**，写入 `STAGING` 镜像和全部载荷，回读 CRC；最后仅把状态字从 `STAGING` 改为 `COMMITTED`。这个最后写入是唯一不可逆的提交点。
5. 因此，擦除、传输、掉电或 USB 串口中断发生在提交前，旧槽仍能启动；新镜像不会被误认为可启动。
...
应用的编译 ABI 必须与目标匹配：当前为 `rv32imc` / `ilp32`，硬件 ABI 为 `0x00000005`。镜像工具和启动器都会拒绝 ABI、入口地址、长度或 CRC 不匹配的文件。
...
当前首版启动器没有“运行中的应用主动请求进入更新器”的软件命令；标准入口是复位后的连接窗口。这个选择避免应用崩溃或错误状态妨碍恢复，后续若增加应用侧更新 API，必须保持现有复位恢复路径不变。

...
这是面向硬件、嵌入式软件和测试人员的**工程级完整规格书**。它集中描述当前 MCU 产品位流的 CPU、存储、
全部可用外设、完整已约束顶层引脚、J5 扩展映射、PINMUX 所有权、电气边界和升级路径。应用开发者只需
使用 SDK；不需要直接接触 PicoRV32、Verilog 或 FPGA 配置文件。

面向应用开发者的一份式中文产品摘要、逐针表和编译烧录流程见
[《OpenMCU-TN9K MCU 中文规格书》](datasheet.md)；本页继续作为寄存器与 RTL 对齐的工程级完整合同。
...
| 0.6 | 2026-08-25 | 完成 12 路扩展 GPIO、UART1、PWM1、TIMER1、诊断/返 Bootloader、P0 外置器件 SDK 与资源受控 RV32M 除法；建立外设与引脚工程级完整合同。 |

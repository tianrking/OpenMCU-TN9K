...
> **硬件 ABI：** `0x0000_0006`（0.6）
> **定位：** 一次固化 FPGA 平台，之后以独立 MCU 固件持续升级应用
> **证据状态：** RTL、SDK、自动化回归与开源 P&R 已完成；实体 Tang Nano 9K HIL 尚待执行。
...
| 客户固件 | `rv32imc` / `ilp32` 裸机应用，产物为 `.omcu` | Linux、RTOS、特权模式或安全启动 |
...
| P1 GPIO/UART/PWM/TIMER | 已实现 | 12 路外扩 GPIO、UART1、四路 PWM1、TIMER1 捕获/正交 RTL、SDK、已编译固件和仿真已覆盖；引脚电平及共线 HIL 待做。 |
| P1 诊断与返 Bootloader | 已实现 | RESET_CAUSE、RUN_TICKS、RESET_COUNT、软件请求/确认、Boot ROM、SDK 与顶层仿真已覆盖；实体复位与 UART 会话 HIL 待做。 |
| FPGA 产品版 | 已完成 P&R | 目标器件已完成综合、布局布线和打包；本次发布的哈希、资源和时序以 `omcu_tn9k_mcu_manifest.json` 为准。 |
...
`build/tangnano9k-mcu-abi06-final/omcu_tn9k_mcu_manifest.json` 记录本次 ABI 0.6 产品构建：

| 项目 | 记录值 |
| --- | --- |
| `.fs` SHA-256 | `6d7973037b53a33ba1a9f7e113f0ca0ad6acb0b45700ba595ad2842213ccdd9f` |
| 时钟 | 27.000 MHz 约束，40.356754 MHz 实现，12.258037 ns 裕量 |
| LUT4 / DFF | 6,844 / 8,640（79.21%）；2,154 / 6,480（33.24%） |
| BSRAM / IOB | 26 / 26（100%）；15 / 276（5.43%） |
| Boot ROM 链 | 4 个 BSRAM；综合与 P&R 初始化指纹一致。 |

这个 manifest 是本次工具运行的可追溯证据，**不是**实体板 HIL 记录。
...
| 应用 ISA | `RV32IMC`，小端，`ilp32`，无浮点、向量、原子扩展、虚拟内存或特权模式承诺。 |
| 乘除法 | 乘法使用硬件快速乘法器；`DIV/DIVU/REM/REMU` 由资源受控的 32 步 PCPI 除法器实现，并有直接 RTL 与已编译 `rv32imc` 固件回归。 |
...
Tang Nano 9K 产品顶层的 `SYSCTRL.FEATURES` 应报告 `0x0000_7FFF`：bit 0..14 全部存在。
较小的 bring-up 顶层不提供 User Flash 时，软件必须先做特性探测，不能假定产品外设必然存在。

| 外设 | 基址 | 核心能力 | 主要限制 |
| --- | ---: | --- | --- |
| GPIO0 | `0x4000_0000` | 6 LED 逻辑 GPIO + 12 路外扩 GPIO、OE、高阻、边沿 IRQ | 不是 ADC/高速采样；只公开已审查的 J5 路由。 |
...
| WDT0 | `0x4000_5000` | 超时 IRQ / SoC 复位请求 | 实体板复位、电源和寿命待测。 |
| PWM0 | `0x4000_6000` | 单路边沿对齐 PWM | 不是功率级或互补驱动器。 |
| IRQCTRL | `0x4000_7000` | 8 个外部源的 pending/enable/force/优先级 | 非 PLIC；固定 PicoRV32 IRQ ABI。 |
...
| PINMUX | `0x4000_B000` | UART1/PWM1/TIMER1 显式获取复用 pad | GPIO、RGB LCD 与复用外设不可同时使用。 |
...
`GPIO0[0..5]` 对应板载低有效 LED。外部 GPIO 为逻辑 GPIO0..11，对应 GPIO0 寄存器 bit 6..17，
总共 12 路；其中 GPIO0..2 位于 J5.8..10，GPIO3..11 位于 J5.11..19，且后者与 RGB LCD 共线。
J6/HDMI 的低电压/高速路径、JTAG、配置相关 pin 和“未使用 IOB”不构成公开 GPIO 合同。

| 功能 | 外部逻辑 GPIO | J5 | package pad | PINMUX 后的用途 |
| --- | --- | --- | --- | --- |
| 普通 GPIO | GPIO0..2 | 8..10 | 28 / 29 / 30 | 仅 GPIO。 |
| 普通或 PWM1 | GPIO3..7 | 11..15 | 33 / 34 / 40 / 35 / 41 | PWM1 CH0..3 使用 GPIO4..7。 |
...
IRQCTRL 把以下 8 个源映射到 PicoRV32 CPU IRQ bit 8..15：
...
- BSRAM 已使用 26/26，不增加大 FIFO、缓存、DMA 缓冲、帧缓冲或 QSPI XIP 到本基线。

...
完整 P0/P1 加可靠性/诊断档案的最终产品构建已完成。真实哈希、工具版本、时序和资源以 ABI 0.9 的 `build/tangnano9k-mcu-abi09-independent-history-filter-packed/omcu_tn9k_mcu_manifest.json` 为准：

| 资源 | 已用 / 总量 | 结论 |
| --- | ---: | --- |
| LUT4 | 7,184 / 8,640（83.15%） | 已成功 P&R；仍须把每项新增组合逻辑作为独立资源与时序变更复验，不能从百分比推断“必定放得下”。 |
| DFF | 2,610 / 6,480（40.28%） | 本次将 96 个产品 GPIO 历史位转为可见的独立输入条件化；仍有数量余量，但 DFF 不能单独替代 LUT、扇出、布线和时序预算。 |
...
| 时序 | 27.000 MHz 约束 | 44.295 MHz 实现频率，14.461 ns 裕量；只证明本次开源 P&R，不能替代实体板测量。 |

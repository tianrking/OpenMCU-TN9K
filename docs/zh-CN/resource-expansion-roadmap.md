...
> **适用基线：** `omcu_tn9k_mcu_top`、硬件 ABI `0.8`、
> `GW1NR-LV9QN88PC6/I5`（GW1N-9C）
> **状态：** P0/P1 与本页新增可靠性/诊断能力均已完成 RTL、SDK、数字回归以及目标器件 P&R/packing；实体板电气、总线目标和 User Flash HIL 仍为发布门禁，不能因综合、仿真或 P&R 通过而标为已验证。
...
完整 P0/P1 加可靠性/诊断档案的最终产品构建已完成。真实哈希、工具版本、时序和资源以 ABI 0.8 的 `omcu_tn9k_mcu_manifest.json` 为准：

| 资源 | 已用 / 总量 | 结论 |
| --- | ---: | --- |
| LUT4 | 6,962 / 8,640（80.58%） | 已成功 P&R；仍须把每项新增组合逻辑作为独立资源与时序变更复验，不能从百分比推断“必定放得下”。 |
| DFF | 2,511 / 6,480（38.75%） | 小型状态机、同步、滤波和诊断状态仍有数量余量；DFF 不能单独替代 LUT、扇出、布线和时序预算。 |
| BSRAM | 24 / 26（92.31%） | 留有两个 BSRAM，但它们不是未评审的 FIFO/DMA/帧缓冲承诺；新增存储架构仍须重新 P&R 与 HIL。 |
| ALU | 1,336 / 6,480（20.62%） | 记录用，不应替代 LUT/时序/布局决策。 |
| MULT36X36 | 1 / 5（20.00%） | 快速乘法器保留。 |
| IOB | 15 / 276（5.43%） | 封装余量不等于 Tang 板上可安全暴露的引脚数。 |
| 时序 | 27.000 MHz 约束 | 40.533 MHz 实现频率，12.366 ns 裕量；只证明本次开源 P&R，不能替代实体板测量。 |
...
| 关闭压缩指令 `C` | 释放实现资源，保留完整 RV32I/RV32M | 应用必须用 `rv32im` 重编译；旧 ABI 0.7 / `rv32imc` 镜像由 Bootloader 拒绝。 |
...
| GPIO 扩展档案 | 顶层/OE/IRQ 位宽，低 | 12 路 GPIO（寄存器 bit0..11），LED0..5 镜像 GPIO0..5，J5.8..19 受控映射 | RTL、CST、固件仿真、ABI 0.8 P&R 已完成；电压/RGB 共线 HIL 待做 | **P1 完成（预 HIL）** |
...
| GPIO 两级同步 / 端口滤波 / 事件快照 | LUT 低到中；不加 BRAM | 全部 12 路同步、共享 N+1 稳定窗口、边沿 IRQ、普通/FAULT 强制快照 | RTL、SDK、寄存器、示例与 ABI 0.8 P&R 已覆盖；按键/传感器/噪声 HIL 待做 | **已实现（预 HIL）** |
| 多路硬件比较定时器 | LUT 低到中；不加 BRAM | ALARM0 复用 TIMER0 时基，提供两路并行 16-bit compare、独立 enable/pending/periodic/IRQ | RTL、SDK、fabric 回归与 ABI 0.8 P&R 已覆盖；HIL 待做 | **已实现（预 HIL）** |
| 脉冲计数 / 频率测量 | LUT 低到中；不加 BRAM | PULSE0 从 GPIO0..2 单选一路，16-bit count/period/last tick | RTL、SDK、pinmux 回归与 ABI 0.8 P&R 已覆盖；输入范围与前端 HIL 待做 | **已实现（预 HIL）** |
| 硬件故障锁存 | LUT 低；不加 BRAM | FAULT0 锁存、PWM0/PWM1 low、全部 GPIO high-Z、共享快照 | RTL、SDK、fabric/top 回归与 ABI 0.8 P&R 已覆盖；不能称功能安全认证，HIL 待做 | **已实现（预 HIL）** |
| 增强看门狗 | LUT 低；不加 BRAM | pretimeout、窗口、最多 8 heartbeat、拒绝喂狗状态 | RTL、SDK、监督状态回归与 ABI 0.8 P&R 已覆盖；真实复位时序 HIL 待做 | **已实现（预 HIL）** |
...
ABI 0.8 当前完成到 **C**。D 至 G 必须使用实际 Tang Nano 9K、所选外设模块、当前板卡 revision 和
...
6. GPIO 全端口滤波/快照、ALARM 同 tick、PULSE 频率/输入选择、FAULT 门控/清除拒绝、WDT 预警/窗口/heartbeat；
7. 温度、电压、长线和重复擦写矩阵。
...
| DMA、QSPI XIP、逻辑分析仪、帧缓冲 | 不进入当前基线 | 虽有两个 BSRAM 余量，但尚无经过评审的存储架构、带宽/一致性合同与 HIL 证据。 |
